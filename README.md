...
기능별 설계·상태·재개 로드맵은 [docs/dev/00-overview.md](docs/dev/00-overview.md)에서 시작합니다.
새 클론 또는 새 세션에서 필요한 비추적 데이터·외부 의존성·스모크 검증은
[docs/dev/06-resume-runbook.md](docs/dev/06-resume-runbook.md)를 따릅니다.
...
- TEMIS 내보내기: `export_temis_topic_cases` (opt-in, 아래 "TEMIS 연동" 참고)
...
- 대량(bulk) 수집/추출(`collect-disclosures`, `bulk-audit-documents`)은
  **CLI 전용**입니다 — 장시간 실행되는 블로킹 MCP 도구는 두지 않습니다.
  ...
  MCP 도구를 사용합니다.
- 이 저장소는 temis DB에 아무것도 쓰지 않습니다. `manifest.json`/
  `bulk-manifest.json`은 로컬 파일일 뿐이며, temis로의 import는(있다면)
  temis 쪽에서 별도로 진행하는 작업입니다(이 저장소는 구현하지 않으며
  완료를 주장하지 않습니다).
...
- `--corp-cls`, `--limit`으로 대상 범위를 좁힐 수 있고, `--emit-topic-cases
  PATH`를 주면 같은 실행 안에서 temis `DartTopicCase` JSON도 함께
  생성합니다(아래 "temis 연계 주의" 참고).
...
`DartTopicCase`(위 "TEMIS 연동" 섹션의 5단계 산출물)는 이 3단계
...
## TEMIS 연동: 안전한 내보내기 흐름

이 저장소는 OpenDART 호출·파싱·변환을 전담하는 adapter/exporter입니다. temis는
OpenDART를 직접 호출하지 않고, 이 저장소가 만든 JSON 파일을 소비합니다. export는
항상 **회사 1건** 단위 산출물이며(5단계), temis가 이를 소비하는 방식은 현재의
...
5. **TEMIS 토픽 케이스 내보내기**
   ...
   출력은 temis `DartTopicCase` 스키마와 호환되는 JSON 배열입니다.

6. **temis에 연결** — 5단계의 산출물은 항상 **회사 1건** 단위입니다(실행할
   때마다 `output_path`를 통째로 덮어씀). temis가 이 파일을 소비하는 방식은
   다음 두 단계로 나뉩니다.

   **현재(파일 기반, 지금 바로 동작):** temis 쪽 환경 변수를 설정하면
   temis가 `DART_TOPIC_CASES_PATH`가 가리키는 파일을 직접 읽습니다.
   ...
   ⚠️ 이 모드에서는 파일 하나에 **회사 1건만** 담깁니다 — temis가 파일을
   그대로 읽기 때문에, 다른 회사의 export로 같은 경로를 재실행하면 이전 회사
   데이터는 사라집니다. 여러 회사를 동시에 다루려면 아래 "목표" 모드가
   필요합니다.

   **목표(DB import, temis 쪽에서 별도로 진행 중):** temis는 각 회사별
   ...
   갖춰지면 `DART_TOPIC_CASES_PATH`는 temis가 매 요청마다 읽는 런타임 파일이
   아니라 import 배치의 **입력(ingestion input)** 파일이 되어, 여러 회사의
   export를 누적할 수 있습니다. 이 DB import 작업은 **temis 쪽에서 별도로
   진행 중인 작업**입니다 — 이 저장소는 구현하지 않으며 완료를 주장하지
   않습니다. 여기서는 방향만 명시합니다.

   어느 모드든 **temis는 별도의 OpenDART adapter가 필요 없습니다.** 이
   저장소가 OpenDART 수집·파싱·`DartTopicCase` 변환을 전담하며, temis는 이
   저장소가 `dart temis-topic-cases` 명령(또는 `export_temis_topic_cases` MCP
   도구)으로 만든 JSON 파일을 읽거나 import하기만 합니다. 향후 temis 쪽에

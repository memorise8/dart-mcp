...
**저장소:** dart-search-mcp (스크립트) · **상태:** ✅ 상장사 1차 생성 · 스크립트는 미커밋(scratchpad)
...
## 실행 (재사용 스크립트)
`scratchpad/nice_yaksik_export.py` (repo 재사용, repo 미수정). 
```bash
uv run python nice_yaksik_export.py 20  /tmp/sample.jsonl      # 샘플 검증
uv run python nice_yaksik_export.py all dart_collected/nice_yaksik_financials.jsonl   # 전체
```
회사당 API 2콜(company + fnlttSinglAcnt), ~6,000콜(일일한도 내). CSV도 생성됨(UTF-8-sig).
...
2. **재사용 CLI 승격**: 현재 스크립트 → `dart nice-yaksik-export --facts ... -o ...` 커밋 기능으로.

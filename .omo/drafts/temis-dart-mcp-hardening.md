...
intent: clear
pending-action: none
approach: Harden dart-search-mcp as the OpenDART source adapter for TEMIS without changing temis in this repo-local plan.
---
...
| C4 | TEMIS-compatible DART topic case JSON can be generated from OpenDART records so temis does not need a separate DART adapter. | active | `.omo/evidence/task-6-temis-dart-mcp-hardening.json` |
...
| TEMIS schema target | Mirror `../../temis/fino-backend/app/schemas/dart_topic_search.py` for generated JSON. | Current TEMIS backend already validates this shape. | Yes |
...
| temis integration boundary | Make `dart-search-mcp` own the OpenDART adapter role and export `dart_topic_cases.json`; temis only points `DART_TOPIC_CASES_PATH` at that file. | The current temis backend already consumes a JSON file, so duplicating OpenDART adapter logic inside temis would increase coupling and maintenance. | Yes |
...
- `../../temis/fino-backend/app/schemas/dart_topic_search.py:10` defines the TEMIS `DartTopicCase` fields this repo should export toward.
...
- Generate a TEMIS-compatible JSON artifact instead of planning a temis OpenDART adapter. This keeps repo boundaries clean: this repo owns collection/transformation, while temis consumes the generated file through `DART_TOPIC_CASES_PATH`.
- If later runtime orchestration is needed, keep it to a thin job/config layer that invokes this MCP/export command; do not duplicate OpenDART request/parsing logic in temis.
...
- Explicit adapter boundary documentation: `dart-search-mcp` is the DART adapter/exporter; temis is a JSON consumer.
- Optional live smoke commands that require `DART_API_KEY` but never expose the key.
- Compatibility validation against the current temis `DartTopicCase` schema by mirroring the required fields.

## Scope OUT (Must NOT have)
- No production temis code changes in this repo.
- No temis-side OpenDART adapter implementation in this plan.

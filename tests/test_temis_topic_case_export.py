...
def _expected_discriminator(fact: AuditReportRecord) -> str:
    """`temis_export`가 문서화한 discriminator 공식을 그대로 재현한다:
    sha256(auditor|audit_opinion|special_matter|emphasis_matter|core_audit_matter)의
    앞 8 hex 문자."""
    source = "|".join(
        (
            fact.auditor,
...
def _expected_case_id(fact: AuditReportRecord, *, topic_slug: str) -> str:
    corp_code = fact.corp_code.strip()
    fiscal_year = int(fact.bsns_year.strip())
    rcept_no = fact.rcept_no.strip()
    return f"dart-{corp_code}-{fiscal_year}-{rcept_no}-{topic_slug}-{_expected_discriminator(fact)}"
...
class ConvertAuditReportsToTopicCasesTests(unittest.TestCase):
    def test_put_option_case_maps_all_fields_with_strict_types(self) -> None:
        ...
        self.assertEqual(record.case_id, _expected_case_id(fact, topic_slug="put-option"))
    ...
    def test_revenue_case_matches_fixture_style_ordering(self) -> None:
        ...
        self.assertEqual(len(records), 1)
        record = records[0]
        self.assertEqual(record.case_id, _expected_case_id(fact, topic_slug="revenue"))
        self.assertEqual(record.topic_tags, ["수익인식", "보증", "계약부채"])

    def test_multiple_facts_same_topic_and_year_get_unique_case_ids(self) -> None:
        ...
        self.assertEqual(
            case_ids,
            [
                _expected_case_id(fact_a, topic_slug="put-option"),
                _expected_case_id(fact_b, topic_slug="put-option"),
            ],
        )
    ...
    def test_no_keyword_match_uses_general_slug_and_empty_tags(self) -> None:
        ...
        self.assertEqual(record.case_id, _expected_case_id(fact, topic_slug="general"))
...
class CaseIdUniquenessTests(unittest.TestCase):
    """Fix 1: case_id는 소스 값에서만 계산되어 전역적으로 고유하고,
    입력 순서와 무관하며, 재생성해도 항상 동일해야 한다."""

    def test_same_topic_and_year_different_companies_get_different_case_ids(self) -> None:
        fact_a = _fact(corp_code="00126380", corp_name="삼성전자", special_matter="풋옵션 관련 사항.")
        fact_b = _fact(corp_code="00164779", corp_name="현대자동차", special_matter="풋옵션 관련 사항.")
        ...
        self.assertNotEqual(records[0].case_id, "dart-put-option-2024-001")
        self.assertNotEqual(records[1].case_id, "dart-put-option-2024-001")

    def test_case_id_is_order_independent(self) -> None:
        fact_a = _fact(rcept_no="20250312000001", special_matter="풋옵션 관련 사항.")
        fact_b = _fact(rcept_no="20250312000002", special_matter="풋옵션 관련 별도 사항.")
        fact_c = _fact(rcept_no="20250312000003", corp_code="00164779", special_matter="풋옵션 관련 세번째 사항.")

        forward, _ = convert_audit_reports_to_topic_cases(
            [fact_a, fact_b, fact_c], freshness_timestamp=_FRESHNESS
        )
        reversed_order, _ = convert_audit_reports_to_topic_cases(
            [fact_c, fact_b, fact_a], freshness_timestamp=_FRESHNESS
        )

        forward_ids_by_report = {r.report_id: r.case_id for r in forward}
        reversed_ids_by_report = {r.report_id: r.case_id for r in reversed_order}
        self.assertEqual(forward_ids_by_report, reversed_ids_by_report)

    def test_case_id_is_deterministic_across_regenerations(self) -> None:
        fact = _fact(special_matter="풋옵션 관련 사항.")

        first, _ = convert_audit_reports_to_topic_cases([fact], freshness_timestamp=_FRESHNESS)
        second, _ = convert_audit_reports_to_topic_cases([fact], freshness_timestamp=_FRESHNESS)

        self.assertEqual(first[0].case_id, second[0].case_id)

    def test_same_report_same_topic_different_content_get_distinct_case_ids(self) -> None:
        fact_a = _fact(
            rcept_no="20250312000001",
            auditor="삼일회계법인",
            special_matter="풋옵션 관련 사항 A.",
        )
        fact_b = _fact(
            rcept_no="20250312000001",
            auditor="한영회계법인",
            special_matter="풋옵션 관련 사항 B.",
        )
    ...
    def test_case_ids_globally_unique_across_mixed_batch(self) -> None:
        facts = [
            _fact(corp_code="00126380", rcept_no="20250312000001", special_matter="풋옵션."),
            _fact(corp_code="00126380", rcept_no="20250312000002", special_matter="풋옵션."),
            _fact(corp_code="00164779", rcept_no="20250312000001", special_matter="풋옵션."),
            _fact(corp_code="00164779", bsns_year="2024", rcept_no="20250312000003", special_matter="특이사항 없음."),
            _fact(corp_code="00164779", bsns_year="2023", rcept_no="20250312000004", special_matter="특이사항 없음."),
        ]

        records, skipped = convert_audit_reports_to_topic_cases(facts, freshness_timestamp=_FRESHNESS)
...
class TopicTagMatchingTests(unittest.TestCase):
    """Fix 2: 짧은 키워드가 더 긴 용어의 일부로만 나타날 때의 오탐을 막는다."""

    def test_risk_management_phrase_does_not_get_lease_tag(self) -> None:
        fact = _fact(special_matter="회사의 리스크 관리 체계를 검토하였다.")

        records, skipped = convert_audit_reports_to_topic_cases([fact], freshness_timestamp=_FRESHNESS)

        self.assertEqual(skipped, [])
        self.assertNotIn("리스", records[0].topic_tags)

    def test_lease_deposit_phrase_does_not_get_warranty_tag(self) -> None:
        fact = _fact(special_matter="임차보증금 및 예치금 현황을 확인하였다.")

        records, skipped = convert_audit_reports_to_topic_cases([fact], freshness_timestamp=_FRESHNESS)

        self.assertEqual(skipped, [])
        self.assertNotIn("보증", records[0].topic_tags)

    def test_genuine_lease_mention_gets_lease_tag(self) -> None:
        fact = _fact(special_matter="운용리스 계약에 따른 자산을 검토하였다.")

        records, skipped = convert_audit_reports_to_topic_cases([fact], freshness_timestamp=_FRESHNESS)

        self.assertEqual(skipped, [])
        self.assertIn("리스", records[0].topic_tags)

    def test_genuine_warranty_mention_gets_warranty_tag(self) -> None:
        fact = _fact(special_matter="제품 보증 채무를 인수하였다.")

        records, skipped = convert_audit_reports_to_topic_cases([fact], freshness_timestamp=_FRESHNESS)

        self.assertEqual(skipped, [])
        self.assertIn("보증", records[0].topic_tags)

    def test_multi_keyword_text_tags_all_matched_keywords(self) -> None:
        fact = _fact(
            special_matter=(
                "풋옵션과 금융부채, 운용리스 계약, 손상 검토, 공정가치 측정을 모두 확인하였다."
            )
        )
        ...
        self.assertEqual(skipped, [])
        self.assertEqual(
            records[0].topic_tags,
            ["풋옵션", "금융부채", "손상", "공정가치", "리스"],
        )

    def test_unknown_topic_falls_back_to_general_with_empty_tags(self) -> None:
        fact = _fact(special_matter="특이사항 없음.")

        records, skipped = convert_audit_reports_to_topic_cases([fact], freshness_timestamp=_FRESHNESS)

        self.assertEqual(skipped, [])
        self.assertEqual(records[0].topic_tags, [])


class ExtractionConfidenceTests(unittest.TestCase):
    """Fix 3: extraction_confidence는 핵심 필드 완전성을 반영해야 하며
    매칭된 키워드 개수를 반영해서는 안 된다."""

    def test_all_key_fields_populated_yields_high_confidence(self) -> None:
        fact = _fact(
            auditor="삼일회계법인",
            audit_opinion="적정",
        ...
        self.assertEqual(skipped, [])
        self.assertEqual(records[0].extraction_confidence, 1.0)

    def test_mostly_empty_or_dash_fields_yield_low_confidence(self) -> None:
        fact = _fact(
            auditor="-",
            audit_opinion="적정",
            core_audit_matter="",
        ...
        self.assertEqual(skipped, [])
        self.assertEqual(records[0].extraction_confidence, 0.25)

    def test_confidence_stays_within_bounds(self) -> None:
        empty_fact = _fact(auditor="-", audit_opinion="-", core_audit_matter="-", emphasis_matter="-")
        full_fact = _fact(
            auditor="삼일회계법인",
            audit_opinion="적정",
            core_audit_matter="핵심감사사항.",
            emphasis_matter="강조사항.",
        )

        records, skipped = convert_audit_reports_to_topic_cases(
            [empty_fact, full_fact], freshness_timestamp=_FRESHNESS
        )
    ...
    def test_keyword_dense_but_field_sparse_fact_does_not_get_top_confidence(self) -> None:
        keyword_dense_fact = _fact(
            auditor="-",
            audit_opinion="적정",
            special_matter="풋옵션 금융부채 비지배지분 수익인식 보증 계약부채 계속기업 우발부채",
            core_audit_matter="",
            emphasis_matter="",
        )

        records, skipped = convert_audit_reports_to_topic_cases(
            [keyword_dense_fact], freshness_timestamp=_FRESHNESS
        )

        self.assertEqual(skipped, [])
        self.assertTrue(len(records[0].topic_tags) >= 5)
        self.assertLess(records[0].extraction_confidence, 1.0)
